...
export function DropdownWhite({
  ...
  return (
    ...
    >
      <a
        className="font-medium text-slate-100 dark:text-slate-400 hover:text-blue-600 px-3 lg:px-5 flex items-center transition duration-150 ease-in-out"
        href="#0"
        aria-expanded={dropdownOpen}
        onClick={(e) => e.preventDefault()}
      >
      ...
      <Transition
        show={dropdownOpen}
        as="ul"
        className="origin-top-right absolute top-full right-0 w-40 bg-white ml-4 rounded shadow-lg"
        enter="transition ease-out duration-200 transform"
        enterFrom="opacity-0 -translate-y-2"
        enterTo="opacity-100 translate-y-0"
        leave="transition ease-out duration-200"
